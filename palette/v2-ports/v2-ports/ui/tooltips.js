 ...
 *   3. Emotion tooltips also render on the circumplex, hover/tap
 ...
 *
 * v2 delta: input source is no longer Muse EEG.
 *   Facial expression and heart rhythm (camera rPPG) replace the four
 *   EEG lanes. Copy for control tips updates accordingly. Structure,
 *   placement logic, and toggle behavior are preserved verbatim.
 *
 * @author  Eyal Gever
 ...
 * `science` (a single sentence of grounding) so information-dense tips
 * can teach as well as guide.
 */
export const STATIC_TIPS = {
  ...
  "face-signal": {
    ...
    body: "Real-time reading of your face by a machine-learning model running on your device. Valence rises with pleasant expressions, arousal with intensity of activation. No video ever leaves the browser.",
    sig: "Model: Human ML \u00b7 on-device \u00b7 60\u2013120ms latency",
  },
  "heart-signal": {
    title: "Heart rhythm",
    body: "Your pulse read from the tiny colour changes in the skin of your face, no wearable required. The baseline locks after roughly a minute, then the field breathes with the beat.",
    sig: "Camera rPPG \u00b7 50\u201390 bpm typical \u00b7 baseline lock ~60s",
  },
  "play-pause": {
    title: "Play / pause",
  ...
  "map": {
    title: "Emotion map",
    body: "Overlay the emotion wheel on top of the artwork. Your feeling paints a chromatic trail as it drifts, and the artwork underneath shifts to the same colour.",
  },
  "close-map": {
    title: "Close the map",
    body: "Return to the artwork alone.",
  },
  "sensing": {
    title: "Sensing",
    body: "Open the sensing panel: what the camera is reading right now, your heart rhythm, and the shader inputs each signal is driving.",
  },
  "clean": {
    title: "Clean",
    body: "Hide the overlays and stay with the artwork. The feeling word remains, everything else recedes.",
  },
  "session-help": {
    title: "How this works",
    body: "Feel with the artwork. Tap the Map to see the circumplex the algorithm is reading right now. Tap the voice record button to capture a spoken note.",
  },
  "summary-help": {
    title: "Session replay",
    body: "Watch the whole journey play back, the artwork re-runs at the same emotion, your heart rhythm breathes with it, and your voice note replays at the moment you spoke.",
  },
  "replay-timeline": {
    title: "Timeline",
  ...
  // Per-style descriptions. One-sentence poetic + one-sentence mechanical.
  "style-breath": {
    title: "Breath",
  ...
  /* Session vocabulary. Opened by a small "i" glyph next to the meta
     line on Session Complete and Session Replay. One popover, all
 ...
 * biosignature, and one line of neuroscience each. Preserved from v1 so
 * the artist's voice is preserved.
 */
...
// Optional lightweight debug hook. Host app may call setDebug(fn) to
// route tooltip logs into its own log surface. Default is no-op so this
// module has zero dependencies.
let _dbg = () => {};
export function setTooltipDebug(fn) { _dbg = typeof fn === "function" ? fn : () => {}; }

// Tooltip DOM + placement

let tipEl = null;
let currentAnchor = null;
// Identity key. When the tooltip is open, `currentKey` holds the
// `data-ea-tip` value of the anchor that opened it. Source of truth for
// the toggle. Comparing keys instead of DOM refs is bulletproof against
// iOS Safari's quirks: pointerover/pointerout synthesised from touch,
// event target being a child <span> vs the <button>, hideTip's timer
// racing with the next tap, and the circumplex layer clearing
// currentAnchor via showTipAtPoint.
let currentKey = null;
let hideTimer = null;
// pinned = user tapped an anchor to open the tooltip. While pinned,
// hover-out / pointer-out DO NOT close the tooltip. It only closes
// when the user (a) taps the same anchor again, (b) taps outside any
// anchor and outside the tip itself, or (c) presses ESC.
let pinned = false;
...
/** Show tooltip at an absolute client point (used for SVG / canvas hits) */
...
/**
 * Programmatically show an emotion tooltip (used by the circumplex hit
 * layer, which uses canvas/SVG hit targets not covered by [data-ea-tip]).
 *
