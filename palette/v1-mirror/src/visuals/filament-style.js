...
void main() {
  ...
  // Denser filament network than Paper's default so on a small
  // chamber the pattern reads as fine nerves, not big chambers.
  // Openness fundamentally changes the wire structure:
  // open (1.0) → fewer, wider, breathier wires; scale contracts so
  //              each "cell" occupies more of the screen.
  // closed (0.0) → dense intricate lattice, scale expands so many
  //                fine filaments pack the space.
  // Range: 1.8× (fully open) .. 5.0× (fully closed)
  float density = mix(1.8, 5.0, 1.0 - uOpenness);
  uv *= density;
  ...
  // Openness also shapes the edge softness: open → soft fat glowing
  // veins (low gamma exponent); closed → crisp razor filaments (high).
  float edgeSharpness = mix(0.5, 7.5, 1.0 - uOpenness);
  noise = pow(noise, 0.7 + edgeSharpness * uContrast);
  noise = min(0.85, noise);

  float blend = smoothstep(0.30, 0.85, noise);
  vec3 filamentColour = mix(uColorMid, uColorFront, blend);

  float safeNoise = max(noise, 0.0);
  vec3 col = filamentColour * safeNoise * 0.9 + uColorBack * (1.0 - clamp(safeNoise, 0.0, 1.0));
...
function compile(gl, type, src) {
  ...
  if (!gl.getShaderParameter(s, gl.COMPILE_STATUS)) {
    ...
    gl.deleteShader(s);
    throw new Error("Filament shader compile failed: " + info);
  }
  return s;
}
function link(gl, vs, fs) {
  ...
  if (!gl.getProgramParameter(p, gl.LINK_STATUS)) {
    throw new Error("Filament program link failed: " + gl.getProgramInfoLog(p));
  }
  return p;
}
...
export class FilamentStyle {
  static id = "filament";
  static name = "Filament";
  static subtitle = "Living nerves lit from within";
  static tech = "webgl2";
  ...
  setEmotion(v, a, o, _label) {
    ...
    // Filament reads as living wires — pull the harmonic chord and
    // blend the front stop toward the neighbour hot bleed so the
    // brightest crossings carry a whisper of the adjacent emotion.
    // Same 3-stop shader as before; the neighbour lives inside the
    // front colour rather than as a fifth uniform.
    const chord = harmonicPalette(v, a, {
      saturationBoost: 1.5,
      front:   0.88,
      hot:     0.90,
      hotShift: 0.30, // subtle neighbour tilt on the hot node
    });

    this._colMidT   = chord.mid;
    // Only 25% neighbour bleed in the bright accent so filament
    // still reads primarily as the primary emotion.
    this._colFrontT = [
    ...
    const mid = chord.mid; // used by the CSS var below

    // Arousal → brightness (hotter crossings when energised).
    // Openness → passed directly to shader as uOpenness so it can
    // reshape the wire density and edge sharpness simultaneously.
    // Contrast is now a fixed moderate value — openness owns the
    // structural feel, brightness owns the intensity.
    this._brightnessT = 0.15 + 0.35 * clamp01(0.5 * (a + 1));
